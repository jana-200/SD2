...
public class Tree {
	...
	public void computeMinimaxValues() {
		...
		// Récupération des valeurs minimax des enfants
		Triplet leftVal = leftChild.getMinimaxValue();
		Triplet rightVal = rightChild.getMinimaxValue();

		// Détermination de la valeur minimax du nœud courant
		if (state.isBlueToPlay()) {
			this.minimaxValue = minOrange(leftVal, rightVal); // le bleu joue → minimise l'orange
		} else {
			this.minimaxValue = minBlue(leftVal, rightVal);   // l'orange joue → minimise le bleu
		}
	}
